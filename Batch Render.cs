 ...
 **/
using System;using System.IO;using System.Text;using System.Drawing;using System.Collections;using System.Collections.Generic;using System.Diagnostics;using System.Windows.Forms;
...
public class EntryPoint {
	...
	String defaultBasePath = "Untitled_";const
	int QUICKTIME_MAX_FILE_NAME_LENGTH = 55;
	...
	void DoBatchRender(ArrayList selectedTemplates, String basePath, RenderMode renderMode)
    {
        ...
        foreach (RenderItem renderItem in selectedTemplates)
        {
			...
			String filename =  Path.Combine(outputDirectory,  //<- uncomment if all projects have region names
            ...
            if (RenderMode.Regions == renderMode) {
                ...
                foreach (ScriptPortal.Vegas.Region region in myVegas.Project.Regions) {
					...
					String regionFilename = String.Format("{0}{1}{2}",
					filename.TrimEnd(), //<- uncomment if all projects have region names
					region.Label,
					renderItem.Extension);
	...
	DialogResult ShowBatchRenderDialog() {
		...
		RenderProjectButton = AddRadioControl(dlog, "Render Project", 6, buttonTop, true);
		RenderSelectionButton = AddRadioControl(dlog, "Render Selection", RenderProjectButton.Right, buttonTop,
				(0 != myVegas.SelectionLength.Nanos));
		RenderRegionsButton = AddRadioControl(dlog, "Render Regions", RenderSelectionButton.Right, buttonTop,
	...
	static Guid[] TheDefaultTemplateRenderClasses = { Renderer.CLSID_SfWaveRenderClass, Renderer.CLSID_SfW64ReaderClass,
			Renderer.CLSID_CSfAIFRenderFileClass, Renderer.CLSID_CSfFLACRenderFileClass,
